        ...
        return (
          <div key={entry.id} className="bg-surface-container rounded-sm p-4 space-y-3">
            ...
            <div className="flex items-center justify-between gap-2">
              <span className={[
                'inline-flex items-center gap-1 px-2 py-0.5 rounded-sm text-[10px] tracking-wide',
                isActive
              ...
              <span className="text-[10px] text-on-surface-variant">{entry.period}</span>
            </div>
            {/* Role */}
            <div>
              <p className="text-sm text-on-surface font-medium">{entry.role}</p>
              <p className="text-[10px] text-primary mt-0.5">{entry.company}</p>
            </div>
            {/* Bullets */}
            <ul className="space-y-1.5">
              {entry.bullets.map((bullet) => (
                <li key={bullet} className="flex gap-2 text-on-surface-variant leading-relaxed">
            ...
            {idx === 0 && (
              <div className="pt-2 border-t border-outline/20 flex flex-wrap gap-3 text-[10px] text-on-surface-variant">
